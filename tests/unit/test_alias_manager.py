...
class TestAliasManager:
    ...
    def test_provider_prefix_handling(self):
        ...
        with patch.dict(os.environ, {
            ...
            assert alias_manager.resolve_alias('anthropic-fast') == 'anthropic:claude-3-5-haiku-20241022'
    ...
    def test_logging_behavior(self, caplog):
        ...
        with patch.dict(os.environ, {
            ...
            with caplog.at_level('INFO'):
                ...
                assert any('Loaded model alias: valid -> openai:gpt-4o-mini' in record.message for record in caplog.records)

                # Check that invalid alias was logged with warning
                assert any('Invalid alias configuration for VDM_ALIAS_INVALID=invalid@format' in record.message for record in caplog.records)

                # Check that empty alias was logged with warning
                assert any('Empty alias value for VDM_ALIAS_EMPTY' in record.message for record in caplog.records)